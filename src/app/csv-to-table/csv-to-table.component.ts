    ...
    this._http.get(this.ApiUrl.csvFile)
      .subscribe(data => {
        ...
        for (let i = 1; i < this.csvData.length - 1; i++) {
          ...
          this.ELEMENT_DATA.push({ position: value[0], English: value[1], Hindi: value[2], Definition: value[3], StrongNumber: value[4], References: value[5], AlignedStrongs: (strongAligned == '') ? 'NA' : strongAligned.replace(/\)/g, "").replace(/\./g, ""), HindiAlignedWords: (hindiAligned == '') ? 'NA' : hindiAligned.replace(/\)/g, "").replace(/\./g, "") })
